...
media::ffmpeg_mp4::ffmpeg_mp4(std::string &&n, ff_image_args &&img, ff_audio_args &&aud)
:_tmp(std::string(n).replace(n.find("mp4"), 3, "")), name(n), image(img), audio(aud),
f_rgb_name(_tmp + "rgb"), f_264_name(_tmp + "h264"), f_pcm_name(_tmp + "pcm"), f_aac_name(_tmp + "aac"), pts(0),
...
af_ctx(nullptr), ac_ctx(nullptr), a_stm(nullptr), a_frm(nullptr) {
    ...
    log_d("video_ctx created. [v:%d,%d,%d,%d],[a:%d,%d,%d],\n%s\n%s\n%s\n%s\n%s.",
...
void media::ffmpeg_mp4::encode_image_frame(int32_t w, int32_t h, uint32_t *data) {
    ...
    sws_scale(i_sws_ctx, i_rgb_frm->data, i_rgb_frm->linesize, 0, h, i_yuv_frm->data, i_yuv_frm->linesize);
}

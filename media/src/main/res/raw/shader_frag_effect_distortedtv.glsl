...
//    vec2 fragCoord = gl_FragCoord.xy;
    ...
    float red 	= texture(s_Texture, vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
    float green = texture(s_Texture, vec2(uv.x + xOffset,	                y)).g+staticVal;
    float blue 	= texture(s_Texture, vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
